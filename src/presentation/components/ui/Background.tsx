...
const BR = 56, BG = 189, BB = 248;

interface Wave { x: number; y: number; t: number; alive: boolean; }
interface Cell { col: number; row: number; idx: number; z: number; }
interface Projected { px: number; py: number; scale: number; }

function project(cx: number, cy: number, z: number, w: number, h: number): Projected {
...
const Background = () => {
  ...
  useEffect(() => {
    ...
    const onMove  = (e: MouseEvent) => { mx = e.clientX; my = e.clientY; lastMove = performance.now(); isIdle = false; };
    const onLeave = () => { mx = -9999; my = -9999; lastMove = performance.now() - IDLE_DELAY; };
    ...
    // Pause RAF quand l'onglet est caché → économie CPU/GPU
    const onVisibility = () => {
      if (document.hidden) {
    ...
    const loop = () => {
      ...
      gAlpha = isIdle ? Math.max(0, gAlpha - IDLE_SPD) : Math.min(1, gAlpha + 0.09);
      ...
      for (let col = 0; col < cols; col++)
        for (let row = 0; row < rows; row++)
          cells.push({ col, row, idx: col * rows + row, z: zElev[col * rows + row] });
      ...
      for (const { col, row, idx, z } of cells) {
        ...
        if (!hasZ) {
          ...
          const pts:  Projected[] = [project(x0,y0,z,W,H), project(x1,y0,z,W,H), project(x1,y1,z,W,H), project(x0,y1,z,W,H)];
          const pts0: Projected[] = [project(x0,y0,0,W,H), project(x1,y0,0,W,H), project(x1,y1,0,W,H), project(x0,y1,0,W,H)];
          const lift = z / MAX_Z;
          const a = Math.min(1, 0.18 + lift * 0.82);

          ctx.fillStyle = `rgba(${BR},${BG},${BB},${(lift * 0.13).toFixed(3)})`;
          ctx.beginPath();
          ctx.moveTo(pts[0].px,pts[0].py); ctx.lineTo(pts[1].px,pts[1].py);
          ctx.lineTo(pts[2].px,pts[2].py); ctx.lineTo(pts[3].px,pts[3].py);
          ...
          ctx.beginPath();
          ctx.moveTo(pts[0].px,pts[0].py); ctx.lineTo(pts[1].px,pts[1].py);
          ctx.moveTo(pts[0].px,pts[0].py); ctx.lineTo(pts[3].px,pts[3].py);
          ctx.stroke();
          ...
          ([0,1,2,3] as const).forEach(i => {
            ...
            ctx.lineWidth = 0.5;
            ctx.beginPath();
            ctx.moveTo(pts[i].px,pts[i].py); ctx.lineTo(pts0[i].px,pts0[i].py);
            ctx.stroke();
          });
          ...
          if (b > 0.02) {
            ...
            ctx.lineWidth = 0.6;
            ctx.beginPath();
            ctx.moveTo(pts0[0].px,pts0[0].py); ctx.lineTo(pts0[1].px,pts0[1].py);
            ctx.moveTo(pts0[0].px,pts0[0].py); ctx.lineTo(pts0[3].px,pts0[3].py);
            ctx.stroke();
          }
        }
      }
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none">
      <div className="blob w-96 h-96 rounded-full top-[-10%] left-[-10%]" style={{ background: "#38bdf8" }} />
      <div className="blob w-96 h-96 rounded-full bottom-[-10%] right-[-10%]" style={{ background: "#818cf8", animationDelay: "-5s" }} />
      <canvas
        ref={canvasRef}
        aria-hidden="true"
        tabIndex={-1}
